...
const parsePasswordString = passwordString => {
  ...
  const pwRangeArr = splitArr[0].split('-');
...
const checkPasswordValidity2 = parsedPwObj => {
  let counter = 0;
  const pw = parsedPwObj.password,
    char = parsedPwObj.charToCheck,
    posArr = parsedPwObj.pwRangeArr;
  // checking if char is at the substring position1
  if (pw.substring(posArr[0] - 1, 1) === char) counter++;
  if (pw.substring(posArr[1] - 1, 1) === char) counter++;
...
export default function () {
  ...
  const inputArr = inputs.year20day2.split('\n');
  ...
  inputArr.forEach(ele => {
    ...
    if (checkPasswordValidity1(parsedStringObj)) {
      validPwCount1++;
    }
    if (checkPasswordValidity2(parsedStringObj)) {
      validPwCount2++;
    }
  });
